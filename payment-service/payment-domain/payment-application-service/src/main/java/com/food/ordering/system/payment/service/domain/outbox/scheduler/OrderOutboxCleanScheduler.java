...
public class OrderOutboxCleanScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderOutboxMessage>> orderOutboxMessagesOptional = orderOutboxHelper.getOrderOutboxMessageByOutboxStatus(OutboxStatus.COMPLETED);
        if(orderOutboxMessagesOptional.isPresent() && orderOutboxMessagesOptional.get().size() > 0) {
            ...
            log.info("Deleted {} OrderOutboxMessage!", orderOutboxMessagesOptional.get().size());
        }
    }
}
