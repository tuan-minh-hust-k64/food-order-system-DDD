...
public class PaymentResponseKafkaListener implements KafkaConsumer<Envelope> {
    ...
    public PaymentResponseKafkaListener(PaymentResponseMessageListenerImpl paymentResponseMessageListener, OrderMessagingDataMapper orderMessagingDataMapper, KafkaMessageHelper kafkaMessageHelper) {
        ...
        messages.forEach(avroModel -> {
            if (avroModel.getBefore() == null && DebeziumOp.CREATE.getValue().equals(avroModel.getOp())) {
                ...
                PaymentOrderEventPayload paymentOrderEventPayload =
                        kafkaMessageHelper.getEventPayload(paymentResponseAvroModel.getPayload(), PaymentOrderEventPayload.class);
                try {
                    if (PaymentStatus.COMPLETED.name().equals(paymentOrderEventPayload.getPaymentOrderStatus())) {
                    ...
                    } else if (PaymentStatus.CANCELLED.name().equals(paymentOrderEventPayload.getPaymentOrderStatus()) ||
                            PaymentStatus.FAILED.name().equals(paymentOrderEventPayload.getPaymentOrderStatus())) {
